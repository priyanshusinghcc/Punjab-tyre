...
function BrandGrid({
  ...
  title: string;
  items: { name: string; stock: string }[];
}) {
  return (
    <div>
      <div className="mb-4 flex items-center gap-3">
        <h3 className="text-sm font-bold text-black">{title}</h3>
        <Badge className="bg-black/8 text-black/50 text-[10px]">
          {items.length} brands
        </Badge>
      </div>
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
        {items.map((item) => (
          ...
          >
            ...
            <div className="mt-0.5 text-[11px] text-black/40">{item.stock}</div>
          </div>
        ))}
      </div>
    </div>
    ...
    <section className="container-shell py-20">
      <FadeUp>
        ...
        <p className="mt-3 max-w-lg text-sm leading-relaxed text-black/50">
          We stock genuine tyres from India's most trusted manufacturers — for
          every vehicle type, usage, and budget.
        </p>
      </FadeUp>
